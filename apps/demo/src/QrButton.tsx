...
export function QrButton() {
  ...
  useEffect(() => {
    ...
    import('qrcode').then(QR =>
      QR.toDataURL(href, { margin: 1, width: 320, color: { dark: '#22c55e', light: '#0a0a0a' } })
  ...
  return (
    <>
      <button style={btnStyle} onClick={() => setOpen(true)} aria-label="Show QR code">
        scan on mobile
      </button>
      ...
      {open && (
        <div style={overlayStyle} onClick={() => setOpen(false)} role="dialog" aria-modal="true">
          <div style={panelStyle} onClick={e => e.stopPropagation()}>
            ...
            {isLocalhost && (
              <div style={hintStyle}>
                <strong>localhost won't reach your phone.</strong><br />
                run <code style={codeStyle}>pnpm --filter @load-games/demo dev -- --host</code><br />
...
const panelStyle: React.CSSProperties = {
  position: 'relative', background: '#0a0a0a', border: '1px solid #22c55e',
  borderRadius: 4, padding: '20px 20px 16px', maxWidth: 360, width: '100%',
  fontFamily: 'monospace', color: '#fff',
}
const closeStyle: React.CSSProperties = {
  position: 'absolute', top: 4, right: 8, background: 'transparent',
  border: 'none', color: '#666', cursor: 'pointer', fontSize: 22, lineHeight: 1, padding: 4,
}
...
const qrFrameStyle: React.CSSProperties = {
  background: '#0a0a0a', padding: 4, borderRadius: 4,
...
const urlStyle: React.CSSProperties = {
  ...
  marginTop: 12, padding: '6px 8px', background: '#050505',
  border: '1px solid #1a1a1a', borderRadius: 3,
}
const hintStyle: React.CSSProperties = {
  marginTop: 12, padding: 10, background: '#050505',
  border: '1px solid #2a2a2a', borderRadius: 3,
  fontSize: 11, color: '#888', lineHeight: 1.5,
}
const codeStyle: React.CSSProperties = {
  background: '#1a1a1a', padding: '1px 5px', borderRadius: 2, color: '#22c55e',
}
